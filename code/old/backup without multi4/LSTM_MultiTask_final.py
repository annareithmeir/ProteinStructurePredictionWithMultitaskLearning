...
if(DSSP_MODE==8 and MAP_TO_DSSP3=='True'):
    LOG_PATH='log/'+MULTITASK_MODE+'/'+str(DSSP_MODE)+'_mapped/LSTM_'+INPUT_MODE+'_'+str(DSSP_MODE)+'_multitask'
else:
    LOG_PATH = 'log/'+MULTITASK_MODE+'/' + str(DSSP_MODE) + '/LSTM_' + INPUT_MODE + '_' + str(DSSP_MODE) + '_multitask'
STATS_PATH='stats'
...
model=Networks.MultiLSTM(INPUT_MODE, DSSP_MODE, 7, 32, 2,device).to(device)
...
torch.save(model.state_dict(), LOG_PATH+'/lstm_multitask.ckpt')
