...
class Pipeline():
    def __init__(self, dssp_mode, device, model, opt, crits, lstm_flag, multi_mode, multi_dssp_mode):
        ...
        self.multi_dssp_mode=multi_dssp_mode
    ...
    def trainNet(self, train_loader):
        ...
        for i, (X, Y_struct3_true, Y_struct8_true, Y_solvAcc_true, Y_flex_true, mask_struct3, mask_struct8, mask_solvAcc, mask_flex) in enumerate(train_loader): # iterate over batches
            ...
            Y_struct3_true = Y_struct3_true.to(self.device)
            Y_struct8_true = Y_struct8_true.to(self.device)
            ...
            mask_struct3=mask_struct3.to(self.device)
            mask_struct8 = mask_struct8.to(self.device)
            ...
            self.opt.zero_grad()
            Y_struct3_raw, Y_struct8_raw, Y_solvAcc_raw, Y_flex_raw = self.model(X)

            if(self.multi_mode=='multi2'):
                loss_struct3 = self.crits[0](Y_struct3_raw, Y_struct3_true)
                loss_struct3 *= mask_struct3
                loss_struct3 = loss_struct3.sum() / float(mask_struct3.sum())  # averages the loss over the structure sequences

                loss_struct8 = self.crits[0](Y_struct8_raw, Y_struct8_true)
                loss_struct8 *= mask_struct8
                loss_struct8 = loss_struct8.sum() / float(mask_struct8.sum())  # averages the loss over the structure sequences
                ...
                if(self.multi_dssp_mode==True):
                    loss=loss_struct3+loss_struct8+loss_solvAcc
                elif(self.dssp_mode==3):
                    loss = loss_struct3 + loss_solvAcc
                elif(self.dssp_mode==8):
                    loss = loss_struct8 + loss_solvAcc

            elif(self.multi_mode=='multi3'):
                loss_struct3 = self.crits[0](Y_struct3_raw, Y_struct3_true)
                loss_struct3 *= mask_struct3
                loss_struct3 = loss_struct3.sum() / float(
                    mask_struct3.sum())  # averages the loss over the structure sequences

                loss_struct8 = self.crits[0](Y_struct8_raw, Y_struct8_true)
                loss_struct8 *= mask_struct8
                loss_struct8 = loss_struct8.sum() / float(
                    mask_struct8.sum())  # averages the loss over the structure sequences
                ...
                if (self.multi_dssp_mode == True):
                    ...
                    loss = loss_struct3 + loss_solvAcc + loss_flex
                elif (self.dssp_mode == 8):
                    loss = loss_struct8 + loss_solvAcc +loss_flex
            else:
                loss_struct3 = self.crits[0](Y_struct3_raw, Y_struct3_true)
                loss_struct3 *= mask_struct3
                loss_struct3 = loss_struct3.sum() / float(
                ...
                if (self.multi_dssp_mode == True):
                    loss = loss_struct3 + loss_struct8
                elif (self.dssp_mode == 3):
                    loss = loss_struct3
                elif (self.dssp_mode == 8):
                    loss = loss_struct8
    ...
    def testNet(self, data_loaders, final_flag, log_path):
        ...
        with torch.no_grad():
            total_losses_test=[0,0,0,0,0]
            avg_losses_test=[0,0,0,0,0] #[struct,solvAcc,flex,sum]
            ...
            def evaluateTrainLoss():
                with torch.no_grad():
                    total_losses_train=[0,0,0,0,0]
                    avg_losses_train=[0,0,0,0,0]
                    ...
                    for X_train, Y_struct3_train,Y_struct8_train, Y_solvAcc_train, Y_flex_train, mask_struct3_train,mask_struct8_train, mask_solvAcc_train, mask_flex_train in train_loader:  # iterate over batches
                        ...
                        Y_struct3_true_unmasked_train = Y_struct3_train.to(self.device)
                        Y_struct8_true_unmasked_train = Y_struct8_train.to(self.device)
                        Y_solvAcc_true_unmasked_train=Y_solvAcc_train.to(self.device)
                        Y_flex_true_unmasked_train = Y_flex_train.to(self.device)
                        mask_struct3_train = mask_struct3_train.to(self.device)
                        mask_struct8_train = mask_struct8_train.to(self.device)
                        ...
                        Y_struct3_raw_train, Y_struct8_raw_train, Y_solvAcc_raw_train, Y_flex_raw_train = self.model(X_train)

                        if(self.multi_mode=='multi2'):
                            loss_struct3_train = self.crits[0](Y_struct3_raw_train, Y_struct3_true_unmasked_train)
                            loss_struct3_train *= mask_struct3_train
                            loss_struct3_train = loss_struct3_train.sum() / float(mask_struct3_train.sum())  # averages the loss over the structure sequences
                            total_losses_train[0] += loss_struct3_train.item()

                            loss_struct8_train = self.crits[0](Y_struct8_raw_train, Y_struct8_true_unmasked_train)
                            loss_struct8_train *= mask_struct8_train
                            loss_struct8_train = loss_struct8_train.sum() / float(mask_struct8_train.sum())  # averages the loss over the structure sequences
                            total_losses_train[1] += loss_struct8_train.item()
                            ...
                            total_losses_train[2] += loss_solvAcc_train.item()

                            if(self.multi_dssp_mode==True):
                                total_losses_train[4] += loss_solvAcc_train.item() + loss_struct3_train.item()+ loss_struct8_train.item()
                            if (self.dssp_mode == 3):
                                total_losses_train[ 4] += loss_solvAcc_train.item() + loss_struct3_train.item()
                            if (self.dssp_mode == 8):
                                total_losses_train[4] += loss_solvAcc_train.item() + loss_struct8_train.item()

                        elif(self.multi_mode=='multi3'):
                            loss_struct3_train = self.crits[0](Y_struct3_raw_train, Y_struct3_true_unmasked_train)
                            loss_struct3_train *= mask_struct3_train
                            loss_struct3_train = loss_struct3_train.sum() / float(
                                mask_struct3_train.sum())  # averages the loss over the structure sequences
                            total_losses_train[0] += loss_struct3_train.item()

                            loss_struct8_train = self.crits[0](Y_struct8_raw_train, Y_struct8_true_unmasked_train)
                            loss_struct8_train *= mask_struct8_train
                            loss_struct8_train = loss_struct8_train.sum() / float(
                                mask_struct8_train.sum())  # averages the loss over the structure sequences
                            total_losses_train[1] += loss_struct8_train.item()
                            ...
                            if (mask_flex_train.sum() != 0):
                                ...
                                total_losses_train[3] += loss_flex_train.item()
                                if (self.multi_dssp_mode == True):
                                    total_losses_train[4] += loss_solvAcc_train.item() + loss_struct3_train.item() + loss_struct8_train.item()+ loss_flex_train.item()
                                if (self.dssp_mode == 3):
                                    total_losses_train[4] += loss_solvAcc_train.item() + loss_struct3_train.item() + loss_flex_train.item()
                                if (self.dssp_mode == 8):
                                    total_losses_train[4] += loss_solvAcc_train.item() + loss_struct8_train.item() + loss_flex_train.item()
                            ...
                            if (self.multi_dssp_mode == True):
                                total_losses_train[4] +=loss_struct3_train.item() + loss_struct8_train.item()
                            if (self.dssp_mode == 3):
                                total_losses_train[4] +=  loss_struct3_train.item()
                            if (self.dssp_mode == 8):
                                total_losses_train[4] += loss_struct8_train.item()

                        if(self.dssp_mode==3):
                            Y_struct_raw_train=Y_struct3_raw_train
                            Y_struct_true_unmasked_train=Y_struct3_true_unmasked_train
                            mask_struct_train=mask_struct3_train
                        else:
                            Y_struct_raw_train = Y_struct8_raw_train
                            Y_struct_true_unmasked_train = Y_struct8_true_unmasked_train
                            mask_struct_train = mask_struct8_train

                        Y_struct_pred_unmasked_train = torch.argmax(Y_struct_raw_train.data,
                    ...
                    avg_losses_train[4] = round((total_losses_train[4] / len(train_loader)),3)  # avg loss over residues
            ...
            for X, Y_struct3_true, Y_struct8_true, Y_solvAcc_true, Y_flex_true, mask_struct3, mask_struct8, mask_solvAcc, mask_flex in test_loader: # iterate over batches
                ...
                Y_struct3_true_unmasked = Y_struct3_true.to(self.device)
                Y_struct8_true_unmasked = Y_struct8_true.to(self.device)
                ...
                if(self.multi_mode=='multi2'):
                    loss_struct3 = self.crits[0](Y_struct3_raw, Y_struct3_true_unmasked)
                    loss_struct3 *= mask_struct3
                    loss_struct3 = (loss_struct3.sum()) / float(mask_struct3.sum())
                    total_losses_test[0] += loss_struct3.item()

                    loss_struct8 = self.crits[0](Y_struct8_raw, Y_struct8_true_unmasked)
                    loss_struct8 *= mask_struct8
                    loss_struct8 = (loss_struct8.sum()) / float(mask_struct8.sum())
                    total_losses_test[1] += loss_struct8.item()

                    loss_solvAcc = self.crits[1](Y_solvAcc_raw, Y_solvAcc_true_unmasked)
                    loss_solvAcc *= mask_solvAcc
                    loss_solvAcc = loss_solvAcc.sum() / float(mask_solvAcc.sum())  # averages the loss over the structure sequences
                    total_losses_test[2] += loss_solvAcc.item()

                    if (self.multi_dssp_mode == True):
                        total_losses_test[4] += loss_solvAcc.item() + loss_struct3.item() + loss_struct8.item()
                    if (self.dssp_mode == 3):
                        total_losses_test[4] += loss_solvAcc.item() + loss_struct3.item()
                    if (self.dssp_mode == 8):
                        total_losses_test[4] += loss_solvAcc.item() + loss_struct8.item()
                    ...
                    total_losses_test[3] += loss_flex.item()

                    if (self.multi_dssp_mode == True):
                        total_losses_test[4] += loss_solvAcc.item() + loss_struct3.item() + loss_struct8.item() + loss_flex.item()
                    if (self.dssp_mode == 3):
                        total_losses_test[4] += loss_solvAcc.item() + loss_struct3.item() + loss_flex.item()
                    if (self.dssp_mode == 8):
                        total_losses_test[4] += loss_solvAcc.item() + loss_struct8.item() + loss_flex.item()
                else:
                    loss_struct3 = self.crits[0](Y_struct3_raw, Y_struct3_true_unmasked)
                    loss_struct3 *= mask_struct3
                    loss_struct3 = (loss_struct3.sum()) / float(mask_struct3.sum())
                    ...
                    if (self.multi_dssp_mode == True):
                        total_losses_test[4] += loss_struct3.item() + loss_struct8.item()
                    if (self.dssp_mode == 3):
                        total_losses_test[4] += loss_struct3.item()
                    if (self.dssp_mode == 8):
                        total_losses_test[4] += loss_struct8.item()

                if (self.dssp_mode == 3):
                    Y_struct_raw = Y_struct3_raw
                    Y_struct_true_unmasked=Y_struct3_true_unmasked
                    mask_struct=mask_struct3
                else:
                    Y_struct_raw = Y_struct8_raw
                    Y_struct_true_unmasked = Y_struct8_true_unmasked
                    mask_struct=mask_struct8
            ...
            avg_losses_test[4] = round((total_losses_test[4] / len(test_loader)), 3)  # avg loss over total residues
            ...
            if (final_flag and self.multi_mode!='struct'): #plot solvAcc and Flex for one random sample for checking if everything works correctly
                ...
                print('R2 ERROR: ', rsquared_flex, rsquared_solvAcc) #something still weird here
