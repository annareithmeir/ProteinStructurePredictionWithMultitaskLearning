...
MAP_TO_DSSP3=sys.argv[4] # True or False
...
assert(MAP_TO_DSSP3=='True' or MAP_TO_DSSP3=='False')

LOG='log/'

if(MAP_TO_DSSP3=='True' and DSSP_MODE==8):
    LOG_PATH = LOG+MULTITASK_MODE+'/' + str(DSSP_MODE) + '_mapped/CNN_' + INPUT_MODE + '_' + str(DSSP_MODE) + '_multitask'
else:
    LOG_PATH=LOG+MULTITASK_MODE+'/'+str(DSSP_MODE)+'/CNN_'+INPUT_MODE+'_'+str(DSSP_MODE)+'_multitask'
    #print(LOG, LOG_PATH)
STATS_PATH='stats'
...
model=Networks.MultiCNN(INPUT_MODE, DSSP_MODE).to(device)
...
for prot in os.listdir(INPUT_PATH):
    if (os.path.exists('preprocessing/'+prot+'/structures_8.npy') and
            os.path.exists('preprocessing/'+prot+'/protvec+scoringmatrix.npy') and
            os.path.exists('preprocessing/' + prot + '/protvec+scoringmatrix.npy') and
            os.path.exists('/home/mheinzinger/contact_prediction_v2/targets/bdb_bvals/'+prot.lower()+'.bdb.memmap') and
            os.path.exists('/home/mheinzinger/contact_prediction_v2/targets/dssp/'+prot.lower()+'/'+prot.lower()+'.rel_asa.memmap')): #only if all input representations available
...
print('MULTITASK MODE: ', MULTITASK_MODE)
...
train_set, test_set, eval_set=DataLoader.train_val_test_split(inputs, targets_structure, targets_solvAcc, targets_flexibility, masks_struct, masks_solvAcc,masks_flex,WEIGHTS, DSSP_MODE)
