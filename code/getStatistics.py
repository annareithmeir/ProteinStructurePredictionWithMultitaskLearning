...
#
# This file does the data analysis and saves all statistics in npy files
...
def readStructures3(protein): #Reads DSSP3
...
def readStructures8(protein): #Reads DSSP8
...
def getInput(classification_mode): #Reads all inputs for one classification mode (3 or 8)
    dict={}
    lengths=[]
    i=0
...
def countStructures3(dict, f): #Counts DSSP3 structures
    c = 0
    h = 0
    e = 0
    xy = 0
...
def countStructures8(dict, f): #Counts DSSP8 structures
    h = 0
    e = 0
    t = 0
    s = 0
...
def countStructureChains3_dict(dict, f): #Counts chains of consecutive DSSP3 states
...
def countStructureChains8_dict(dict, f): #Counts chains of consecutive DSSP8 states
...
def countAminoAcidsPerStruct_dict(dict3, dict8, f): # Counts how often an amino acid occurs under each structure
...
def countDistanceBetweenBetaSheets_dict(dict, f): #Counts the average number of residues between two beta-sheets
...
def analyseFlex(dict): #Analysis of the B-factors
    flex_avgs=[]
    three_bins=[0,0,0]
    for sample in dict.keys():
        flex=dict[sample][3]
...
def analyseSolvAcc(dict): #Analysis of the RSA values
    solv_avgs = []
    four_bins = [0, 0, 0, 0]
    two_bins=[0,0]
    for sample in dict.keys():
...
f = open('data analysis/statistics.txt', 'w')
