...
class Pipeline():
    ...
    def trainNet(self, train_loader):
        ...
        for i, (X, Y_struct3_true, Y_struct8_true, Y_solvAcc_true, Y_flex_true, mask_struct3, mask_struct8, mask_solvAcc, mask_flex, len_wo_padding) in enumerate(train_loader): # iterate over batches
            X = X.to(self.device)
        ...
        with torch.no_grad():
            ...
            for X, Y_struct3_true,Y_struct8_true, Y_solvAcc_true, Y_flex_true, mask_struct3, mask_struct8, mask_solvAcc, mask_flex, len_wo_padding in test_loader: # iterate over batches
                ...
                if(self.multi_mode=='multi4'):
                    if(self.dssp_mode==3):
                        ...
                        if (final_flag):  # last epoch, bootstrapping only over the test samples --> len(bootstrapped_conf_mat)==len(validation_set)
                            ...
                            for i in range(Y_struct_true_unmasked[0].size()[0]):
                                ...
                                '''
                                print(Y_solvAcc_raw.size())
                                print(mask_solvAcc.size())
                                Y_solvAcc_raw_sample = Y_solvAcc_raw[i][0].cpu().numpy()
                                Y_solvAcc_true_sample = Y_solvAcc_true[i][0].cpu().numpy()
                                mask_solvAcc_sample = mask_solvAcc[i][0]
                                Y_solvAcc_raw_sample = Y_solvAcc_raw_sample[mask_solvAcc_sample != 0]
                                Y_solvAcc_true_sample = Y_solvAcc_true_sample[mask_solvAcc_sample != 0]

                                Y_flex_raw_sample = Y_flex_raw[i][0].cpu().numpy()
                                Y_flex_true_sample = Y_flex_true[i][0].cpu().numpy()
                                mask_flex_sample = mask_flex[i][0]
                                Y_flex_raw_sample = Y_flex_raw_sample[mask_flex_sample != 0]
                                Y_flex_true_sample = Y_flex_true_sample[mask_flex_sample != 0]

                                rsquared_flex = r2_score(Y_flex_true_sample, Y_flex_raw_sample)
                                rsquared_solvAcc = r2_score(Y_solvAcc_true_sample, Y_solvAcc_raw_sample)
                                '''
                        ...
                        if (final_flag):  # last epoch, bootstrapping only over the test samples --> len(bootstrapped_conf_mat)==len(validation_set)
                            ...
                            for i in range(Y_struct_true_unmasked[1].size()[0]):
                                ...
                                if (self.dssp_mode == 8 and self.map == 'True'):
                                    ...
                                    confusion_matrix_per_sample = np.zeros((self.dssp_mode, self.dssp_mode),
                                                                           dtype=np.int)
                    ...
                    if (final_flag): # last epoch, bootstrapping only over the test samples --> len(bootstrapped_conf_mat)==len(validation_set) erklaeren warum -outer loop!
            ...
            if(final_flag): # plotting of bootstrapping in last epoch
                ...
                if(self.dssp_mode==8 and self.map=='False'):
                    utilities.write_bootstrapping8(log_path, bootstrapped_conf_mat)
                else:
