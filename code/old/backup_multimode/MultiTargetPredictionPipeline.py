        ...
        for i, (X, Y_struct3_true, Y_struct8_true, Y_solvAcc_true, Y_flex_true, mask_struct3, mask_struct8, mask_solvAcc, mask_flex) in enumerate(train_loader): # iterate over batches
            ...
            if(self.multi_mode=='multi2'):
                ...
                loss_struct3 = loss_struct3.sum() / float(
                    mask_struct[0].sum())  # averages the loss over the structure sequences

                loss_struct8 = self.crits[0](Y_struct_raw[1], Y_struct_true[1])
                loss_struct8 *= mask_struct[1]
                loss_struct8 = loss_struct8.sum() / float(
                    mask_struct[1].sum())  # averages the loss over the structure sequences
    ...
    def testNet(self, data_loaders, final_flag, log_path):
        ...
        with torch.no_grad():
                    ...
                    for X_train, Y_struct3_train,Y_struct8_train, Y_solvAcc_train, Y_flex_train, mask_struct3_train,mask_struct8_train, mask_solvAcc_train, mask_flex_train in train_loader:  # iterate over batches
                        ...
                        if(self.multi_mode=='multi2'):
                            ...
                            loss_struct3_train = loss_struct3_train.sum() / float(
                                mask_struct_train[0].sum())  # averages the loss over the structure sequences
                            ...
                            loss_struct8_train = loss_struct8_train.sum() / float(
                                mask_struct_train[1].sum())  # averages the loss over the structure sequences
                        ...
                        if(self.multi_mode=='multi4'):
                            if(self.dssp_mode==3):
                                Y_struct_raw_train=Y_struct_raw_train[0]
                                Y_struct_true_unmasked_train=Y_struct_true_unmasked_train[0]
                            else:
                                Y_struct_raw_train = Y_struct_raw_train[1]
                                Y_struct_true_unmasked_train=Y_struct_true_unmasked_train[1]
            ...
            for X, Y_struct3_true, Y_struct8_true, Y_solvAcc_true, Y_flex_true, mask_struct3, mask_struct8, mask_solvAcc, mask_flex in test_loader: # iterate over batches
                ...
                if (self.multi_mode == 'multi4'):
                    if (self.dssp_mode == 3):
                        Y_struct_raw = Y_struct_raw[0]
                        Y_struct_true_unmasked=Y_struct_true_unmasked[0]
                    else:
                        Y_struct_raw = Y_struct_raw[1]
                        Y_struct_true_unmasked=Y_struct_true_unmasked[1]
                ...
                #print('---TEST')
                #print(mask_struct.sum(), Y_struct_true.shape, Y_struct_true_accumulate_test.shape)
