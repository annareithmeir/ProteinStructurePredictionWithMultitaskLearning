...
def multiTargetLoss(y_true, y_pred): #https://blog.manash.me/multi-task-learning-in-keras-implementation-of-multi-task-classification-loss-f1d42da5c3f6
...
def trainNet(model, opts, crits, train_loader):
    ...
    Y_true_all=[Y_struct_true_all, Y_solvAcc_true_all, Y_flex_true_all]
    Y_pred_all=[Y_struct_pred_all,Y_solvAcc_pred_all, Y_flex_pred_all]
    return avg_struct_loss_train, avg_solvAcc_loss_train, avg_flex_loss_train, Y_true_all, Y_pred_all
...
def testNet(model, dataloaders, crits, epoch, eval_summary, final_flag):
    ...
    with torch.no_grad():
        ...
        Y_pred_all_test=[Y_struct_pred_all, Y_solvAcc_pred_all, Y_flex_pred_all]
        Y_true_all_test=[Y_struct_true_all, Y_solvAcc_true_all, Y_flex_true_all]
        ...
        def evaluateTrainLoss():
            with torch.no_grad():
                ...
                Y_struct_true_all = np.zeros(0, dtype=np.int)  # collects true targets
                Y_struct_pred_all = np.zeros(0, dtype=np.int)  # collects predicted targets
                Y_solvAcc_true_all = np.zeros(0, dtype=np.int)  # collects true targets
                Y_solvAcc_pred_all = np.zeros(0, dtype=np.int)  # collects predicted targets
                Y_flex_true_all = np.zeros(0, dtype=np.int)  # collects true targets
                Y_flex_pred_all = np.zeros(0, dtype=np.int)  # collects predicted targets
                ...
                for X, Y_struct, Y_solvAcc, Y_flex, mask_struct, mask_solvAcc, mask_flex in train_loader:  # iterate over batches
                    ...
                    Y_solvAcc_pred_unmasked = torch.argmax(Y_solvAcc_raw.data,
                                                          dim=1)  # returns index of output predicted with highest probability
                    Y_solvAcc_pred = Y_solvAcc_pred_unmasked[mask_solvAcc != 0]  # applies weighted mask
                    Y_solvAcc_true = Y_solvAcc_true_unmasked[mask_solvAcc != 0]  # applies weighted mask
                    Y_solvAcc_true = Y_solvAcc_true.view(-1).long().cpu().numpy()
                    Y_solvAcc_pred = Y_solvAcc_pred.view(-1).long().cpu().numpy()
                    ...
                    Y_flex_pred_unmasked = torch.argmax(Y_flex_raw.data,
                                                          dim=1)  # returns index of output predicted with highest probability
                    Y_flex_pred = Y_flex_pred_unmasked[mask_flex != 0]  # applies weighted mask
                    Y_flex_true = Y_flex_true_unmasked[mask_flex != 0]  # applies weighted mask
                    Y_flex_true = Y_flex_true.view(-1).long().cpu().numpy()
                    Y_flex_pred = Y_flex_pred.view(-1).long().cpu().numpy()
                ...
                Y_pred_all=[Y_struct_pred_all, Y_solvAcc_pred_all, Y_flex_pred_all]
                Y_true_all=[Y_struct_true_all, Y_solvAcc_true_all, Y_flex_true_all]
            return avg_losses_train, Y_true_all, Y_pred_all
    ...
    return avg_losses_train, Y_true_all_train, Y_pred_all_train,avg_losses_test, Y_true_all_test, Y_pred_all_test, confusion_matrices
...
print('RAW WEIGHTS FLEX:', WEIGHTS_FLEX)
...
for epoch in range(NUM_EPOCHS):
    ...
    print('acc score structure pred:',round(accuracy_score( train_true_all[0], train_pred_all[0] ),2),round(accuracy_score( test_true_all[0], test_pred_all[0] ),2))
    print('acc score solvAccpred:', round(accuracy_score(train_true_all[1], train_pred_all[1]), 2),round(accuracy_score(test_true_all[1], test_pred_all[1]), 2))
    print('acc score flex pred:', round(accuracy_score(train_true_all[2], train_pred_all[2]), 2),round(accuracy_score(test_true_all[2], test_pred_all[2]), 2))
    train_acc.append(accuracy_score( train_true_all[0], train_pred_all[0] ))
    test_acc.append(accuracy_score( test_true_all[0], test_pred_all[0] ))
...
utilities.get_other_scores_multitask(confusion_matrices_epoch)
