...
INPUT_MODE=sys.argv[1]  #protvec or onehot or protvec_evolutionary or protvec+scoringmatrix/protvec+gapweights/protvec+information
DSSP_MODE=3       #3 or 8
N_SPLITS_KFOLD=3
LOG_PATH='log/Structure/CNN_'+INPUT_MODE+'_'+str(DSSP_MODE)+'_structure'
STATS_PATH='stats'
...
model=Networks.CNN(INPUT_MODE).to(device)
...
pipeline=MultiTargetPredictionPipeline.Pipeline(DSSP_MODE, device, model, opt, criterions, False, False)
...
torch.save(model.state_dict(), LOG_PATH+'/cnn_structure.ckpt')