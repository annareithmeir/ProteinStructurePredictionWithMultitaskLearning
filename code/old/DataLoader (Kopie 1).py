...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, idx):  #addpadding_mask, in test() apply that to preds and write with XY states to pred.txt
        X = self.inputs[idx]
        ...
        if self.max_len:  # if a maximum length (longest seq. in the set) was provided
            n_missing = self.max_len - len(Y_struct3)  # get the number of residues which have to be padded to achieve same size for all proteins in the set
        ...
        return (input_tensor, struct3_tensor, struct8_tensor,  solvAcc_tensor, flex_tensor, mask_struct3_tensor, mask_struct8_tensor,  mask_solvAcc_tensor, mask_flex_tensor)
...
#
def createDataLoaders(train_set, val_set, test_set, batch_size_train, batch_size_test):
    data_loaders = dict()
    ...
    data_loaders['Val'] = torch.utils.data.DataLoader(dataset=val_set,
