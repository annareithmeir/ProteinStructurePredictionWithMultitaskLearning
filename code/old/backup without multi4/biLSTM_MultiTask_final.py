...
assert(MULTITASK_MODE=='struct' or MULTITASK_MODE=='multi2' or MULTITASK_MODE=='multi3')
if(MAP_TO_DSSP3=='True' and DSSP_MODE==8):
    LOG_PATH='log/'+MULTITASK_MODE+'/'+str(DSSP_MODE)+'_mapped/biLSTM_'+INPUT_MODE+'_'+str(DSSP_MODE)+'_multitarget'
else:
    LOG_PATH = 'log/'+MULTITASK_MODE+'/' + str(DSSP_MODE) + '/biLSTM_' + INPUT_MODE + '_' + str(DSSP_MODE) + '_multitarget'
STATS_PATH='stats'
...
model=Networks.MultibiLSTM(INPUT_MODE, DSSP_MODE, 10, 32, 2,device).to(device)
...
pipeline=MultiTargetPredictionPipeline.Pipeline(DSSP_MODE, device, model, opt, criterions, True, MULTITASK_MODE)
...
for epoch in range(NUM_EPOCHS):
    ...
    train_loss_epoch, train_true_all, train_pred_all, test_loss_epoch, test_true_all, test_pred_all, confusion_matrix_epoch = pipeline.testNet(
        data_loaders, final_flag, LOG_PATH)
...
torch.save(model.state_dict(), LOG_PATH+'/bi_lstm_multitask.ckpt')
