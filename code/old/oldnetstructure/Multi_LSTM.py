...
def trainNet(model, opt, crit, train_loader):
    ...
    for i, (X, Y_struct_true_unmasked, Y_solvAcc_true_unmasked, Y_flex_true_unmasked, mask_struct, mask_solvAcc, mask_flex) in enumerate(train_loader): # iterate over batches
        ...
        Y_solvAcc_true_unmasked = torch.transpose(Y_solvAcc_true_unmasked, 1, 2)
        Y_solvAcc_true_unmasked = torch.squeeze(Y_solvAcc_true_unmasked, 2).to(device)
        Y_flex_true_unmasked = torch.transpose(Y_flex_true_unmasked, 1, 2)
        Y_flex_true_unmasked = torch.squeeze(Y_flex_true_unmasked, 2).to(device)

        model.zero_grad()
        model.hidden = model.init_hidden(X.size()[0])

        Y_struct_pred_unmasked, Y_solvAcc_pred_unmasked, Y_flex_pred_unmasked = model(X)
        ...
        loss_solvAcc = crit(Y_solvAcc_pred_unmasked, Y_solvAcc_true_unmasked)
        loss_solvAcc = torch.unsqueeze(loss_solvAcc, 1)
        loss_solvAcc *= mask_solvAcc.to(device)
        loss_solvAcc = loss_solvAcc.sum() / float(mask_solvAcc.sum())  # averages the loss over the structure sequences

        loss_flex = crit(Y_flex_pred_unmasked, Y_flex_true_unmasked)
        loss_flex = torch.unsqueeze(loss_flex, 1)
        loss_flex *= mask_flex.to(device)
        loss_flex = loss_flex.sum() / float(mask_flex.sum())  # averages the loss over the structure sequences

        loss=loss+loss_solvAcc+loss_flex
        #print(loss.item())
        loss.backward()
        opt.step()
...
def testNet(model, dataloaders, crit, epoch, eval_summary, final_flag):
    ...
    with torch.no_grad():
        ...
        for X, Y_struct_true_unmasked, Y_solvAcc_true_unmasked, Y_flex_true_unmasked, mask_struct, mask_solvAcc, mask_flex in test_loader: # iterate over batches
            ...
            total_loss_test+=loss+loss_solvAcc+loss_flex
        ...
        def evaluateTrainLoss():
            with torch.no_grad():
                ...
                for X, Y_struct_true_unmasked, Y_solvAcc_true_unmasked, Y_flex_true_unmasked, mask_struct, mask_solvAcc, mask_flex in train_loader:  # iterate over batches
                    ...
                    total_loss_train=loss+loss_solvAcc+loss_flex
...
DSSP_MODE=3       #3 or 8
N_SPLITS_KFOLD=3
LOG_PATH='log/Multi_LSTM_'+INPUT_MODE+'_'+str(DSSP_MODE)
STATS_PATH='stats'
...
model=ConvNets.MultiLSTM(INPUT_MODE, DSSP_MODE, 6, 32, 1,device).to(device)
