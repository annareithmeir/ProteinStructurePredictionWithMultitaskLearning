...
#
# This file is the biLSTM model and takes the following arguments when being called:
...
if(MAP_TO_DSSP3=='True' and DSSP_MODE==8):
    LOG_PATH='log/'+MULTITASK_MODE+'/'+str(DSSP_MODE)+'_mapped/biLSTM_'+INPUT_MODE+'_'+str(DSSP_MODE)+'_multitask'
else:
    LOG_PATH = 'log/'+MULTITASK_MODE+'/' + str(DSSP_MODE) + '/biLSTM_' + INPUT_MODE + '_' + str(DSSP_MODE) + '_multitask'
...
model=Networks.biLSTM(INPUT_MODE, DSSP_MODE, MULTITASK_MODE, 34, 32, 2,device).to(device)
...
pipeline=MultiTargetPredictionPipeline.Pipeline(DSSP_MODE, device, model, opt, criterions, True, MULTITASK_MODE, MAP_TO_DSSP3)
...
for epoch in range(NUM_EPOCHS):
    ...
    train_loss_epoch, train_true_all, train_pred_all, test_loss_epoch, test_true_all, test_pred_all, confusion_matrices_epoch = pipeline.testNet(
