...
class MultiCNN(nn.Module):
    def __init__(self, input_mode, dssp_mode, multi_mode):
        ...
        if(self.multi_mode=='multi4' or self.dssp_mode==3):
            self.conv3_3 = nn.Conv2d(10, 3, kernel_size_layer3, padding=pad_l3) #Classification Struct
        if(self.multi_mode=='multi4' or self.dssp_mode==8):
            self.conv3_8 = nn.Conv2d(10, 8, kernel_size_layer3, padding=pad_l3)  # Classification Struct
    ...
    def forward(self, x): #x:(32,120,1,968)
        ...
        if(self.multi_mode=='multi4'):
            out_struct3 = F.log_softmax(self.conv3_3(out), dim=1)  # out_struct:(32,3,1,968) --> return as (32,3,968)?
            out_struct8 = F.log_softmax(self.conv3_8(out), dim=1)  # out_struct:(32,3,1,968) --> return as (32,3,968)?
...
class MultiLSTM(nn.Module):
    ...
    def forward(self, x): #x:(32,120,1,968)
        ...
        if(self.multi_mode=='multi4'):
            lstm_out_struct3 = F.log_softmax(self.hidden2structtag_3(lstm_out), dim=2)  # lstm_out_struct:(32,968,3)
            lstm_out_struct8 = F.log_softmax(self.hidden2structtag_8(lstm_out), dim=2)  # lstm_out_struct:(32,968,3)
            out_struct3 = torch.transpose(lstm_out_struct3, 1, 2)  # out_struct:(32,3,968)
            out_struct8 = torch.transpose(lstm_out_struct8, 1, 2)  # out_struct:(32,3,968)
...
class MultiTargetDenseNet(nn.Module):
    def __init__(self, input_mode, dssp_mode, multi_mode):
        ...
        if (self.multi_mode == 'multi4' or self.dssp_mode == 3):
            self.conv3_3 = nn.Conv2d(50, 3, kernel_size_layer3, padding=pad_l3)  # Classification Struct
        if (self.multi_mode == 'multi4' or self.dssp_mode == 8):
            self.conv3_8 = nn.Conv2d(50, 8, kernel_size_layer3, padding=pad_l3)  # Classification Struct
    ...
    def forward(self, x): #x:(32,120,1,968)
        ...
        if (self.multi_mode == 'multi4'):
            out_struct3 = F.log_softmax(self.conv3_3(out4), dim=1)  # out_struct:(32,3,1,968) --> return as (32,3,968)?
            out_struct8 = F.log_softmax(self.conv3_8(out4), dim=1)  # out_struct:(32,3,1,968) --> return as (32,3,968)?
