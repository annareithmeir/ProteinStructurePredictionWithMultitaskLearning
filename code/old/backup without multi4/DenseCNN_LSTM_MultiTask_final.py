...
INPUT_MODE=sys.argv[1]  #protvec or onehot or protvec_evolutionary or pssm
...
if(DSSP_MODE==8 and MAP_TO_DSSP3=='True'):
    LOG_PATH='log/'+MULTITASK_MODE+'/'+str(DSSP_MODE)+'_mapped/DenseHybrid_'+INPUT_MODE+'_'+str(DSSP_MODE)+'_multitarget'
else:
    LOG_PATH = 'log/'+MULTITASK_MODE+'/' + str(DSSP_MODE) + '/DenseHybrid_' + INPUT_MODE + '_' + str(DSSP_MODE) + '_multitarget'
...
model=Networks.MultiDenseCNN_LSTM(INPUT_MODE, DSSP_MODE, 6, 32, 1,device).to(device)
