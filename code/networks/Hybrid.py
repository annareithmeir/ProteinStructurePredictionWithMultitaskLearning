...
#
# This file is the Hybrid model and takes the following arguments when being called:
...
if(DSSP_MODE==8 and MAP_TO_DSSP3=='True'):
    LOG_PATH='log/'+MULTITASK_MODE+'/'+str(DSSP_MODE)+'_mapped/Hybrid_'+INPUT_MODE+'_'+str(DSSP_MODE)+'_multitask'
else:
    LOG_PATH = 'log/'+MULTITASK_MODE+'/' + str(DSSP_MODE) + '/Hybrid_' + INPUT_MODE + '_' + str(DSSP_MODE) + '_multitask'
...
#contains the class distr for weighting
...
model=Networks.Hybrid(INPUT_MODE, DSSP_MODE, MULTITASK_MODE, 34, 32, 1,device).to(device)
...
pipeline=MultiTargetPredictionPipeline.Pipeline(DSSP_MODE, device, model, opt, criterions, True, MULTITASK_MODE, DSSP_MODE)
...
torch.save(model.state_dict(), LOG_PATH+'/hybrid_multitask.ckpt')