...
#
# This file is the DenseHybrid model and takes the following arguments when being called:
...
if(DSSP_MODE==8 and MAP_TO_DSSP3=='True'):
    LOG_PATH='log/'+MULTITASK_MODE+'/'+str(DSSP_MODE)+'_mapped/DenseHybrid_'+INPUT_MODE+'_'+str(DSSP_MODE)+'_multitask'
else:
    LOG_PATH = 'log/'+MULTITASK_MODE+'/' + str(DSSP_MODE) + '/DenseHybrid_' + INPUT_MODE + '_' + str(DSSP_MODE) + '_multitask'
...
# Class distr is saved here
stats_dict=np.load(STATS_PATH+'/stats_dict.npy').item()

if(DSSP_MODE==3):
    LEARNING_RATE= 0.001 #DSSP3
else:
    LEARNING_RATE= 0.005 #DSSP8
...
model=Networks.DenseHybrid(INPUT_MODE, DSSP_MODE, MULTITASK_MODE, 6, 32, 1,device).to(device)
...
torch.save(model.state_dict(), LOG_PATH+'/densehybrid_multitask.ckpt')